...
if (isset($_SESSION['invite_response'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            array_push($content,"<a class='dropdown-trigger btn' href='#' data-target='{$user_id}'>{$user_name}</a>","<ul id='{$user_id}' class='dropdown-content'><li><a href='process?type=admin&admin_type=active&user_id={$user_id}&CSRFtoken={$csrftoken}'>{$user_status}</a></li><li><a href='process?type=admin&admin_type=delete&user_id={$user_id}&CSRFtoken={$csrftoken}''>Delete</a></li></ul>");
        }
    }
    $title = 'All Users';
} else {
